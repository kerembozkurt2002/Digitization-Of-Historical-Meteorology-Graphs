...
H. Birkan Y\i{}lmaz \\ \\
GitHub Repository: \\
\parbox{0.9\textwidth}{\centering\small\url{https://github.com/kerembozkurt2002/Digitization-Of-Historical-Meteorology-Graphs}}
}
\date{June, 2026}
\maketitle{}
\pagenumbering{roman}
...
\textbf{Non-functional requirements.} The desktop binary must run on macOS and Windows. A complete extract on a typical 4\,Mpx scan must finish in under 3 seconds on a laptop without a discrete GPU. Re-extraction with the same calibration must be deterministic. All temporary files written for rotation correction must be cleaned up on exit and on new-image load. The application must function fully offline.
...
\begin{tikzpicture}[node distance=2.3cm, font=\small]
    ...
    \draw[-{Latex[length=2mm]}] ($(u.south) + (0,-0.4)$) -- node[above, font=\scriptsize]{drop image} ($(f.south) + (0,-0.4)$);
...
The system meets the three success criteria stated in Chapter 2. Across the project dataset, approximately 87\,\% of charts required only start/end point marking with zero editing operations, approximately 9\,\% needed a single refinement or fixation, and the remaining approximately 4\,\% required two or more refinements~--- roughly nine out of ten charts are digitized without any curve editing at all. Table~\ref{table:templates} breaks the result down by template, reporting the approximate fraction of charts that the automatic extraction handles without any manual editing.

\begin{table}[thbp]
\vskip\baselineskip
\caption[Per-template result]{Per-template extraction quality. The last column is the approximate fraction of charts in each template that required no manual editing after automatic extraction.}
\begin{center}
\begin{tabular}{|l|l|l|l|c|} \hline
\textbf{Template} & \textbf{Type} & \textbf{Period} & \textbf{Grid color} & \textbf{Approx.\ \% no edit} \\\hline
gunluk-1   & Daily   & 1990s     & yellow/cream  & $\approx 95$\,\% \\\hline
gunluk-2   & Daily   & 1980      & green/olive   & $\approx 93$\,\% \\\hline
gunluk-3   & Daily   & 1980s     & orange        & $\approx 75$\,\% \\\hline
haftalik-1 & Weekly  & 1940s     & orange        & $\approx 92$\,\% \\\hline
haftalik-2 & Weekly  & 1970s     & orange/pink   & $\approx 72$\,\% \\\hline
4\_gunluk-1 & 4-day  & 1940s     & orange        & $\approx 95$\,\% \\\hline
4\_gunluk-2 & 4-day  & 1955      & orange        & $\approx 93$\,\% \\\hline
4\_gunluk-3 & 4-day  & 1945--50s & orange        & $\approx 92$\,\% \\\hline
4\_gunluk-4 & 4-day  & 1950s     & orange/red    & $\approx 90$\,\% \\\hline
\end{tabular}
...
The project produced a working desktop application that converts scanned Kandilli thermograms into machine-readable temperature time series. Classical computer vision, organized around per-template color profiles and a cylindrical-grid calibration, was sufficient to reach an automatic-extraction quality where approximately 87\,\% of charts require no curve editing at all~--- marking the starting and ending points is enough~--- approximately 9\,\% need only a single refinement, and the remaining approximately 4\,\% need two or more targeted corrections in the focused human-in-the-loop UI. The practical impact on the archive is substantial. Manual transcription of a single thermogram chart by a domain expert is estimated at several minutes per chart, which puts the full 1{,}682-chart Kandilli archive at roughly 80--140 hours of uninterrupted human work. With the proposed system, an end-to-end extraction on a 4\,Mpx scan completes in under three seconds, and the editing actions required for the small fraction of difficult charts take well under a minute each. A conservative estimate of the average per-chart effort is on the order of ten seconds, which compresses the same workload to under five hours and brings what was effectively a multi-year manual project into the scope of a single afternoon's reviewing session. Just as importantly, the workflow is reproducible: re-running the pipeline on the same image with the same calibration yields bit-identical CSVs, and every manually edited point is flagged in the export so downstream climate-science users can filter on provenance.
...
\chapter{CALIBRATION JSON SCHEMA}
...
\chapter{PER-TEMPLATE COLOR PROFILES}
